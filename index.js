...
app.use(
    cors({
        origin: ['http://localhost:5173', 'https://product-recommendation-3e40a.web.app'],
        credentials: true,
    }),
)
...
// const uri = `mongodb://${process.env.DB_USER}:${process.env.DB_PASS}@ac-0wuahyo-shard-00-00.hl3uycw.mongodb.net:27017,ac-0wuahyo-shard-00-01.hl3uycw.mongodb.net:27017,ac-0wuahyo-shard-00-02.hl3uycw.mongodb.net:27017/?ssl=true&replicaSet=atlas-wlh8ou-shard-0&authSource=admin&retryWrites=true&w=majority&appName=Cluster0`
...
async function run() {
	try {
		...
		app.post('/queries', verifyFirebaseToken, async (req, res) => {
			...
			const userExists = await queriesCollection.findOne({
				UserEmail: userEmail,
			})
			...
			if (!userExists) {
				return res
					.status(403)
					.send({ message: 'Access denied. Please register first.' })
			}
